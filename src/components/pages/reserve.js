...
import { createMsgAction, postReservation } from '../../redux/reservation/reservationSlice';
...
const Reservations = () => {
  ...
  useEffect(() => {
    ...
    if (createmsg === 'success') {
      ...
      setErrorMessage('Oops! Reservation couldn\'t be created. Can\'t reserve the same motorcycle on the same day');
    }
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (motorcycleIdState === -1 || date === '' || city === '' || status === '') {
  ...
  return (
    <section className="reserve-motorcycle-page">
      ...
      <div className="reserve-main-container">
        ...
        <p>
          This app gives detailed information about the modern motorcycles which can be hired.
        </p>

        <form onSubmit={handleSubmit} className="reserve-form">
          <select defaultValue={chosenMotorcycleId || ''} name="motorcycle_id" id="motorcycle-drop-down" onChange={(e) => setMotorcycleIdState(e.target.value)}>
          ...
          <select name="status" id="status-dropdown" onChange={(e) => setStatus(e.target.value)}>
